...
tabella leggiCorse(int *n_corse, char *nomeFile);
void Menu(void);
...
void Menu(void){
    ...
    tabella tab;
    tabella tab_ordine[ARRIVO + 1];
    char stringa[MAX_C] = {"corse.txt"};

    tab = leggiCorse(&n, stringa);
    tab.ordine = NONORDINATO;

    riempiMatrici(n, tab, tab_ordine, ARRIVO + 1);

    for(i=0; i<(ARRIVO + 1); i++)
    ...
    do{
        ...
        switch(scelta){
            ...
            case 4:
                free(tab.log);
                for(i=0; i<ARRIVO + 1; i++)
                    free(tab_ordine[i].log);

                printf("Inserire nome del file: ");
                scanf("%s", stringa);
                leggiCorse(&n, stringa);
...
tabella leggiCorse(int *n_corse, char *nomeFile){
    ...
    mat.log = malloc(*n_corse * sizeof(entry)); //alloco un certo numero di log a seconda di quanti me ne servono
...
void ricercaBinaria(tabella mat, char *partenza, int l, int r){ //mi basta la binaria visto che ho gia tutti gli ordinamenti
...
void riempiMatrici(int n_corse, tabella mat, tabella *mat_ordine, int n_ordinamenti){
    ...
    for(i=0; i<n_ordinamenti; i++){
        mat_ordine[i].log = malloc(n_corse * sizeof(entry)); //alloco tanti log quante sono le corse
